...
export default function Solution() {
  ...
  const question = async () =>{
    let result = await fetch(`http://localhost:5000/solution/${id}`)
  ...
  const handleSubmit = async(e)=>{
    ...
    if (desc === ''){
      ...
      setnull("")
      let response = await fetch('http://localhost:5000/ans', {
        method: "post",
        body: JSON.stringify({
          email,
          id,
  ...
  const getAnswers = async ()=>{
    let result = await fetch (`http://localhost:5000/ans/${id}`)
    result = await result.json()
    setans(result)
  }
  useEffect(()=>{
